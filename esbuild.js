...
const {
   malinaPlugin
} = require( "malinajs/malina-esbuild" );
...
( async () => {
   ...
   if ( DEV ) {
      ...
      watch( path.join( CWD, 'src', 'client' ), {
         recursive: true
      }, async function () {
         try {
      ...
      watch( path.join( CWD, 'src', 'server' ), {
         recursive: true
      }, async function () {
   ...
   return {
      ...
      setup( b ) {
         b.onResolve( {
            ...
            return {
               path: DEV ? 'server_development.js' : 'server_production.js',
               namespace: 'server'
            }
         } );

         b.onLoad( {
            filter: /^server_development\.js$/,
            namespace: 'server'
         }, ( args ) => {
            return {
               contents: `
